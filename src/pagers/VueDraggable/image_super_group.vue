...
export default {
  ...
  methods: {
    ...
    handleDel(index, id) {
      ...
      const q = this.list2.find((value, index, arr) => {
        return value.id === id;
      });
        ...
        if (count != i) {
          for (let a = 0; a < this.datalist[i].value.length; a++) {
            if (this.datalist[i].value[a].id == this.ids) {
              const str = JSON.stringify(this.datalist[i].value[a]);
    ...
    constrctorProductInfo() {
      const product_data = this.product_data1;
      const arr = [];
      const tarr = [];
      const tarr1 = [];
      const tarr2 = [];
    ...
    myColor() {
      ...
      const color = this.$refs.mycolor1.$el;
    ...
    getProductInfo() {
      getProductInfo({product_id: this.product_id}).then((res)=>{
        if (res.code == ERR_OK&&res.data.product_info.length) {
          const arr = this.product_data1.map((info)=>{
            const item = res.data.product_info.find((e)=>{
              return e.id == info.id;
            });
            if (item) {
              return item;
